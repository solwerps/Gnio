...
const CANON: Record<string, string> = {
  ...
  "RENTA IMPONÍBLE": "RENTA IMPONIBLE", // por si viene con acento mal puesto
};
...
export default function CargarRetencionesIsrPage() {
  ...
  const base = `/dashboard/contador/${usuario}/empresas/${empresaId}/documentos/retenciones/isr`;
  ...
  // ---- Validación NIT RETENIDO (cabecera Excel) vs empresa.nit
  ...
  async function readExcel(file: File) {
    ...
    try {
      ...
      const filtered = out.map((o) => ({
        ...
        "FECHA EMISION": o["FECHA EMISION"] ?? o["FECHA EMISIÓN"] ?? "",
        "TOTAL FACTURA": o["TOTAL FACTURA"] ?? "",
        "RENTA IMPONIBLE": o["RENTA IMPONIBLE"] ?? "",
  ...
  const onSave = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/retenciones/isr/masivo", {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          date: mes,
          retenciones: rows,
          nit_retenido: onlyDigits(empresaNit), // refuerzo opcional en backend
        }),
      });
        ...
        <div className="mx-auto w-full max-w-[1120px]">
          <div className="flex items-center justify-between mb-6">
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight">
              Retenciones ISR / Carga Masiva de Retenciones
            </h1>
            <button
              onClick={handleBack}
                ...
                <table className="min-w-full text-sm">
                  <thead className="bg-slate-900 text-white">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-right">Renta Imponible</th>
                  ...
                  <tbody>
                    {rows.map((r, i) => (
                      <tr key={i} className="border-b border-neutral-100 hover:bg-neutral-50">
                        ...
                        <td className="px-4 py-2 text-right">{String(r["RENTA IMPONIBLE"] ?? "")}</td>
      ...
      {confirmSave && (
        ...
        >
          ...
          <ul className="text-sm leading-7">
            <li><b>Tipo de operación:</b> <span className="text-blue-600 font-semibold">Retenciones de ISR</span></li>
            <li><b>Empresa:</b> <span className="text-blue-600 font-semibold">{empresaNombre || empresaId}</span></li>
            <li><b>Fecha:</b> {monthPretty(mes)}</li>
