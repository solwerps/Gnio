//src/app/dashboard/contador/[usuarios]/empresas/[id]/documentos/retenciones/isr/page.tsx
...
export default function RetencionesIsrPage() {
  ...
  const [rows, setRows] = useState<RetencionISR[]>([]);
  ...
  async function fetchData() {
    ...
    try {
      ...
      const r = await fetch(`/api/retenciones/isr?${q.toString()}`, { cache: "no-store" });
      const j = await r.json();
      ...
      <main className="md:ml-[260px] px-4 sm:px-6 py-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-3xl font-extrabold tracking-tight">
            Retenciones ISR{empresaNombre ? ` / ${empresaNombre}` : ""}
          </h1>
          <button
                ...
                {loading ? (
                  ...
                  rows.map((r) => (
                    <tr key={r.uuid} className="border-b border-neutral-100 hover:bg-neutral-50">
                      ...
                      <td className="px-4 py-2 text-right">Q{r.rentaImponible.toFixed(2)}</td>
                      <td className="px-4 py-2 text-right font-semibold text-emerald-600">Q{r.totalRetencion.toFixed(2)}</td>
                    </tr>
                  ))
                ) : (
                  <tr><td colSpan={8} className="px-4 py-6 text-center text-neutral-400">No hay retenciones para ese mes.</td></tr>
                )}
              </tbody>
            </table>
          </div>
