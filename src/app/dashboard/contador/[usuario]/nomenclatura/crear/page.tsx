...
export default function CrearNomenclaturaContador() {
  ...
  // Paginación sobre filas VISIBLES (respeta colapsado)
  const [pageSize, setPageSize] = useState(10);
  const [page, setPage] = useState(0);

  // Expansión por raíz 1..6
    ...
    (async () => {
      ...
      try {
        ...
        const normalizadas: CuentaRow[] = list.map((r: any, i: number) => ({
          ...
          lockAdd: typeof r.lockAdd === "boolean" ? r.lockAdd : (r.lockRowActions ?? false),
          lockDelete: typeof r.lockDelete === "boolean" ? r.lockDelete : (r.isPlantilla ? true : (r.lockRowActions ?? false)),
        }));
  ...
  const canAdd = (r: CuentaRow) => {
    if (typeof (r as any).lockAdd === "boolean") return !(r as any).lockAdd; // La API manda
    if (r.isPlantilla && r.principalDetalle === "P") return false; // bloqueo extra si no especifica
    return true;
  };
  const canDelete = (r: CuentaRow) => !(r as any).lockDelete;

  // Forza expansión de la raíz a la que pertenece la nueva cuenta
  ...
  const addAfter = (r: CuentaRow) => {
    ...
    setRows((prev) => {
      ...
      const nueva: any = {
        orden: 999999,
        cuenta: nextCuenta ?? "",          // heredada incrementada
        descripcion: "",                   // NO hereda; queda vacía
        debeHaber: r.debeHaber,            // hereda
        principalDetalle: "D",             // detalle
        nivel: (typeof r.nivel === "number" ? r.nivel : 1), // hereda
        tipo: r.tipo ?? "BALANCE_GENERAL", // hereda
        naturaleza: r.naturaleza ?? "REVISAR", // hereda
        isPlantilla: false,
        ...
        // marca para validar edición de jerarquía
  ...
  // ======== VISIBILIDAD POR EXPANSIÓN ========
  const visibleRows = useMemo(() => {
    return rows.filter((r) => {
      const ld = leadingDigit(r.cuenta) as RootId | undefined;
      if (isRootRow(r)) return true; // raíz siempre visible
      if (!ld || !ROOT_IDS.includes(ld)) return true; // si no se puede inferir raíz, no ocultar
  ...
  try {
    const res = await fetch(`/api/nomenclaturas?tenant=${usuario}`, {
      ...
      body: JSON.stringify({
        ...
        cuentas: reindexRows(rows),
      }),
    });
    const data = await res.json();
    if (!res.ok || data?.ok === false) throw new Error(data?.error || "CREATE_FAILED");

    alert(`Guardado (filas: ${data.totalFilas})`);
    // ⬇️ siempre regresar a la lista
                  ...
                  <tbody>
                    {pageRows.map((r) => {
                     ...
                     const rowIsGrey = r.isPlantilla && r.principalDetalle === "P";
                      ...
                      // Botón "Desplegar" para raíces 1..6
                      ...
                      return (
                        <tr key={r.orden} className={cx("border-b", rowIsGrey && "bg-slate-100")}>
                          ...
                          {/* PRINCIPAL/DETALLE -> Desplegar si es raíz; en otros casos, selector P/D */}
                          <td className="px-3 py-2">
                            {isRoot ? (
                              <button
                                className={cx(
                                  "inline-flex items-center gap-2 text-white px-3 py-1 rounded-full",
                                  rootColors[ld!],
                                  "shadow-sm"
                                )}
                          ...
                          {/* CUENTA (bloquear cuando P y respetar lock) */}
                          <td className="px-3 py-2">
                            <input
                          ...
                          {/* DESCRIPCIÓN / DEBEHABER / NIVEL / TIPO / NATURALEZA */}
                          {[
                            ...
                            if (fd==="descripcion") {
                              return (
                                <td key={fd} className={cx("px-3 py-2", disabled && "bg-slate-200")}>
                                  <input
                              ...
                              return (
                                <td key={fd} className={cx("px-3 py-2", disabled && "bg-slate-200")}>
                                  <select className="border rounded px-2 py-1" value={String(value)} disabled={disabled}
                                    ...
                                    {tipoOptions.map(t=><option key={t} value={t}>{t.replaceAll("_"," ")}</option>)}
                                  </select>
                                </td>
                              );
                            }
